...
CpuStart::CpuStart()
{
    lookUp( procValues);//将成员变量procVlalues带入lookUp中，获得数据
}

QTime CpuStart::upTime() const
{
    QTime t(0,0,0);
    for (int var = 0; var < NValues; ++var) {
        t = t.addSecs(int (procValues[var] / 100));
    }
    return t;
}

void CpuStart::statistic(double &user, double &system)//数据统计，详情不计
{
    ...
    for ( int i = 0; i < NValues; i++ )
        totalDelta += values[i] - procValues[i];//两个Value比较？？

    user = userDelta / totalDelta * 100.0;
    system = systemDelta / totalDelta * 100.0;

    for (int var = 0; var < NValues; ++var) {
        procValues[var] = values[var];//这又是搞毛线，更新数据，将统计后的数据代入procValues成员变量中
    }
}

void CpuStart::lookUp(double values[NValues]) const
{
...
#endif
    {
        ...
        for (int var = 0; var < NValues; ++var) {
            values[var] = dummyValues[counter][var];//获取其中某一个数组
        }
        counter = ( counter + 1)
                % ( sizeof ( dummyValues) / sizeof( dummyValues[0] ) );//这句话是干啥哩,打印一下不就清楚了
        qDebug() << counter;
    }
    else{
        ...
        do {
            ...
            line = line.trimmed();//删除多余的空格 原文：Returns a string that has whitespace removed from the start and the end.
            if ( line.startsWith("cpu")){
                const QStringList valueList =
                        line.split(" ",QString::SkipEmptyParts);//以空格为分隔，并跳过空字符串
